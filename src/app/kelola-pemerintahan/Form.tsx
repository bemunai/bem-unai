...
export default function Form({ prevData }: { prevData?: Organisasi }) {
  ...
  useEffect(() => {
    const fetchOrganisasi = async () => {
      ...
      setOrganisasiList(data.filter((org) => org.type === "Kementerian"));
    };
    fetchOrganisasi();
  }, []);
    ...
    <form action={saveAction} className="w-full max-w-lg">
      ...
      <div className="mb-4">
        ...
        <textarea
          id="description"
          name="description"
          defaultValue={prevData?.description || data?.data?.description}
          className="textarea text-black textarea-bordered textarea-secondary w-full h-44"
      ...
      <div className="mb-4">
        <label className="label">
          <span className="text-base label-text flex items-end">
            {prevData ? (
              <span className="ml-2 text-secondary">
                {" "}
                Parent:{" "}
                <p>
                  {organisasiList.find((org) => org.id === prevData?.familyId)
        ...
        <select
          ...
          className="w-full text-black select select-bordered select-secondary"
        >
          <option key="" value="" className="text-gray-500">
            Pilih Parent
          </option>
      ...
      <div className="flex justify-center">
        {pending ? (
          <span className="loading loading-spinner loading-xl text-center mt-5"></span>
        ) : (
          <button className="bg-gradient-to-r w-full from-secondary to-[#ffff] text-primary px-4 py-2 rounded-xl transition duration-300 transform hover:scale-105 hover:from-[#ffff] hover:to-secondary hover:text-black">
